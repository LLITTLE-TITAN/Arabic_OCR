...
def analyze_image_from_uri_prep(image_uri, feature_types: Sequence) -> vision.AnnotateImageResponse:
    client = vision.ImageAnnotatorClient()

    # Loads the image into memory
    with io.BytesIO(image_uri) as image_file:
...
def start(image_uri):
    ...
    response = []
    if flag_ui and flag_sn and flag_pn and flag_rt:
        response.append(total_ui)
        response.append(total_sn)
        response.append(total_pn)
        response.append(total_rt)
        return  response
